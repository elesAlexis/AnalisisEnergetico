...
st.title("Análisis del Panorama Energético en América y Colombia (2020-2024)")  # Este es el título que aparecerá en la página web
# Introducción general del proyecto
st.write("""
Este proyecto tiene como objetivo analizar el comportamiento del sector energético en América, 
...
df_distribution_over_net_production_colombia = SplitDataSet.get_distribution_over_net_production_colombia(df) # Filtra los datos la distribución (en porcentaje) de consumo, pérdidas, exportaciones e importaciones
...
# 1. Producción Neta vs Consumo Final en América
st.plotly_chart(GraphicsView.plot_production_vs_consumption(df_america_2024))
st.write("""
### Producción Neta vs Consumo Final en América (2024)

Este gráfico muestra cómo se compara la electricidad generada en distintos países de América con el consumo final de electricidad. Se destacan las diferencias en la producción de electricidad por país y por tipo de producto (renovables o no renovables).
""")
st.write("---")  # Agrega una línea divisoria


# 2. Evolución del cantidad de Energía Renovable en América
st.plotly_chart(GraphicsView.plot_renewable_trend(df_renewable))
st.write("""
### Evolución del Cantidad de Energía Renovable en América (2020-2024)

En este gráfico de líneas, observamos cómo ha cambiado el cantidad de electricidad renovable a lo largo de los años 2020 a 2024 en América. Este análisis es crucial para entender las políticas energéticas de cada país y su transición hacia fuentes de energía más sostenibles.
""")
st.write("---")  # Línea divisoria

# 3. Evolución del Cantidad de Energía No Renovable en América
st.plotly_chart(GraphicsView.plot_non_renewable_trend(df_non_renewable))
st.write("""
### Evolución del Cantidad de Energía No Renovable en América (2020-2024)
...
st.plotly_chart(GraphicsView.plot_distribution_over_net_production_colombia(df_distribution_over_net_production_colombia))
st.write("""
